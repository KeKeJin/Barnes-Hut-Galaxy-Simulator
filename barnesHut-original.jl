...
function creepHelper(node::Node)
    ...
    elseif node.hasChildren && node.numOfChild == 1
        ...
        # println("velocity before is, ", node.body.velocity)
        velocity = node.body.velocity .+ time .* acceleration
        # println("velocity after is, ", velocity)
        ...
        # println("ac after creep, ", acceleration)
        # println("velocity after creep, ", velocity)
        # println("dot product is, ", dot(acceleration, velocity))
    end
end
